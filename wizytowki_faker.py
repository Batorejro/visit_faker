...
def create_contacts(is_business, ilosc):
    fake = Faker(['it_IT'])
    ...
    for i in range(ilosc):
        if is_business:
            wizyt = BussinessContact(first_name, work_number, email, company, job, last_name)
        else:
            wizyt = BaseContact(first_name, home_number, email)
            contacts.append(wizyt)
...
create_contacts(True, ilosc) # biznesowe kontakty
create_contacts(False, ilosc) # domowe kontakty
